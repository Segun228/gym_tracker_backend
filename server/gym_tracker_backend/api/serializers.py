...
class ExerciseTemplateSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ["id", "name", "muscle_group", "created_at"]
...
class SetReadSerializer(serializers.ModelSerializer):
    class Meta:
        model = Set
        fields = ["id", "weight", "reps", "duration", "order", "created_at"]


class WorkoutExerciseReadSerializer(serializers.ModelSerializer):
    sets = SetReadSerializer(many=True, read_only=True)
    template = ExerciseTemplateSerializer(read_only=True)

    class Meta:
        model = WorkoutExercise
        fields = ["id", "template", "order", "sets", "created_at"]
...
class WorkoutReadSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Workout
        fields = ["id", "note", "duration", "workout_exercises", "created_at"]